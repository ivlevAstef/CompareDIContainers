...
class MockRandomStatementGenerator: NSObject, RandomStatementGenerator {
  ...
  func generate() -> RandomStatementWrapper {
    ...
    let statement = statements[Int(arc4random_uniform(UInt32(statements.count)))]
    return RandomStatementWrapper(statement)
  }
}
